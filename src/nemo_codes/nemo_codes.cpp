...
#include "../nemo/neuron_models/LIFCore.h"
...
void nemo_init(INeuroCoreBase *core, tw_lp * lp){
    core->core_init(lp);
}
void nemo_pre_run(INeuroCoreBase *core, tw_lp *lp){
    core->pre_run(lp);
}
void nemo_forward_event(INeuroCoreBase *core, tw_bf *bf, nemo_message *m, tw_lp *lp){
    core->forward_event(bf,m,lp);
}

void nemo_reverse_event(INeuroCoreBase *core, tw_bf *bf, nemo_message *m,tw_lp *lp){
    core->reverse_event(bf,m,lp);
}
void nemo_commit(INeuroCoreBase *core, tw_bf *bf, nemo_message *m,tw_lp *lp){
    core->core_commit(bf,m,lp);
}
void nemo_finish(INeuroCoreBase *core, tw_lp *lp){
    core->core_finish(lp);
}
